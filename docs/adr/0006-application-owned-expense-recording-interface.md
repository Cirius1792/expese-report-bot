...
Create an application-owned `ExpenseRecordingPort` consumed by Telegram and CLI. The application-layer `ExpenseRecordingUseCase` is its concrete Implementation. The Interface exposes one mode-aware Expense Recording operation. `ONE_SHOT` preserves CLI behavior without Correction; `CONVERSATIONAL` supports Telegram's existing Correction workflow.

`ExpenseRecordingUseCase` invokes `ExtractionPort` and `ExpenseRepositoryPort` and owns Extraction-to-save orchestration, Expense construction, and Correction lifecycle decisions. Driving Adapters retain transport translation and rendering.
